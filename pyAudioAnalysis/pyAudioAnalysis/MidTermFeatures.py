...
from pyAudioAnalysis.pyAudioAnalysis import utilities
...
def mid_feature_extraction(signal, sampling_rate, mid_window, mid_step,
    ...
    #  这里相当于规定每一个中期窗口中，组中有多少个n_frames;  　[中期窗口长度- （一帧短期窗口的长度) ] / 短期窗口位移
    mt_step_ratio = int(round(mid_step / short_step))  # 这里相当于，每组之间的移动步长与组内之间帧移长度，　两者之间的比值；　
    #  中期窗口与短期窗口的位移比率；　用于规定出，每一组在位移时，组与组之间的位移长度；　
    ...
    for i in range(n_feats): #  对每个特征在帧维度上，　在每一组中， 求出所有特征进行求均值和方差;
        cur_position = 0
        num_short_features = len(short_features[i])  # 每一个特征，有多少帧；
