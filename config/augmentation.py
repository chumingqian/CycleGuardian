...
class GroupSpecAugment(torch.nn.Module):
    '''
    Unofficial Implementation of SpecAugment: A Simple Data Augmentation Method for Automatic Speech Recognition
    Paper: https://arxiv.org/pdf/1904.08779.pdf
    Ref. github: https://github.com/pyyush/SpecAugment/blob/219fc6e9ed4838fe9700295700040b1da283c536/augment.py#L10
    ...
    # def __init__(self, policy, zero_mean_normalized=False):
    def __init__(self,specaug_policy='icbhi_group_cl', specaug_mask='mean'):
        super().__init__()
        _log_api_usage_once(self)

        self.policy = specaug_policy
        self.mask = specaug_mask
        ...
        elif self.policy == 'icbhi_group_cl':
            # (bt,3, 84, 118) 此时是200ms 的区间，用于区分wheeze, 块数少一点
            self.W, self.F, self.m_F, self.T, self.p, self.m_T = 0, 22, 2,  20, 1.0, 2

        #  其中 F,mF,分别代表Freq频率上一小块掩码区间的长度，掩码块的个数
        #  T,m_t： 代表时间维度上一小块掩码区间的长度，掩码块的个数


        elif self.policy == 'group_sr22k':
            # (bt,3, 42, 516)， 此时是20ms 的区间，用于区分wheeze, 时间块数多一点， 频率掩码的块数少点；
            self.W, self.F, self.m_F, self.T, self.p, self.m_T = 0, 10, 2, 5, 1.0, 10
    ...
    def time_warp(self):
        ...
        source_control_point_locations = torch.unsqueeze(src_ctr_pts, 0)  # (1, v//2, 2)
        dest_control_point_locations = torch.unsqueeze(dest_ctr_pts, 0)  # (1, v//2, 2)
        warped_spectro, dense_flows = sparse_image_warp(self.mel_spectrogram, source_control_point_locations,
    ...
    def forward(self, img):
        ...
        """
        # self.mel_spectrogram = img # np.array [time, freq, channel]
        self.mel_spectrogram = img  # torch.tensor [channel, freq,  time,]
        #self.mel_spectrogram = self.mel_spectrogram.transpose(2, 1)  # torch.tensor [channel, freq, time]
...
class GroupSpecAugment_v2(torch.nn.Module):
    '''
    Unofficial Implementation of SpecAugment: A Simple Data Augmentation Method for Automatic Speech Recognition
    Paper: https://arxiv.org/pdf/1904.08779.pdf
    Ref. github: https://github.com/pyyush/SpecAugment/blob/219fc6e9ed4838fe9700295700040b1da283c536/augment.py#L10
    ...
    def __init__(self,specaug_policy='icbhi_group_cl', specaug_mask='mean'):
        ...
        # elif self.policy == 'icbhi_group_cl':
        #     self.W, self.F, self.m_F, self.T, self.p, self.m_T = 0, 26, 2, 125, 1.0, 2
        #
    ...
    def time_mask(self):
        ...
        for i in range(self.m_T):
            ...
            # Ensure that tau is greater than t before applying the mask
            if tau > t:
                t0 = random.randint(0, tau - t)  # [0, tau - t)
                self.mel_spectrogram[:, :, t0:t0 + t] = mask_value
            else:
                # Skip the mask application if tau <= t
                print(f"Skipping mask application for iteration {i} because tau ({tau}) <= t ({t})")
...
class BatchSpecAugment(torch.nn.Module):
