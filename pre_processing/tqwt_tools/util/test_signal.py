...
def speech_signal() -> np.ndarray:
    ...
    with open('/home/respecting-god/Github/Lung_project/tqwt_tools-master/tests/resources/speech1.txt') \
...
def music_signal(start_sample: int = 0, end_sample: int = -1) -> np.ndarray:
    """fs = 8820Hz"""
    #_, audio = read(os.path.join(get_package_path().replace('/tqwt_tools/', '/'), 'tests', 'resources', 'testaudio.wav'))

    _, audio = read('/home/respecting-god/Github/Lung_project/tqwt_tools-master/tests/resources/testaudio.wav')
    #_, audio = read('/home/respecting-god/Github/Lung_project/tqwt_tools-master/tests/resources/219_2b2_Ar_mc_LittC2SE.wav')
    audio = audio / np.abs(np.max(audio))
    return resample(audio, int(audio.shape[0]/5))[start_sample:end_sample]



def lung_signal(start_sample: int = 0, end_sample: int = -1) -> np.ndarray:
    """fs = 8820Hz"""
    #_, audio = read(os.path.join(get_package_path().replace('/tqwt_tools/', '/'), 'tests', 'resources', 'testaudio.wav'))

    _, audio = read('/home/respecting-god/Github/Lung_project/tqwt_tools-master/tests/resources/162_1b2_Ar_mc_AKGC417L.wav')
    audio = audio / np.abs(np.max(audio))
    return resample(audio, int(audio.shape[0]/5))[start_sample:end_sample]
