...
# 本项目连接的  yons ，env  torch2.0.1


# v3-11-1  ，将对比学习的方式引入到其中；
# 3-12-1  多视图对比学习的，引入其中；

# 3-13-1, 多尺度 + 聚类对比学习引入其中；

#3-14-1,  多尺度 +  恢复聚类损失 + 对比损失；
#3-14-2,  使用 cnn + transformer Encode 两种模式的结构

# 3-15-1,  一个使用聚类获取全局向量， 另一个使用 Transformer Encode 获取全局向量；

# 3-17-1  四种尺度， +  gate net  with softmax;
# 3-17-2  四种尺度， +  gate net  without  softmax;
...
from  nets.CycleGuardian_v5_1_3_cls2 import  Projector, GroupMixConLoss
...
class Trainer:
    def __init__(self):
        ...
        if self.args.split_method  == 0:
            ...
            print(" this modeling is testing the  device generalization \n ", )
        ...
        # loading checkpoint
        self.net = create_model(num_classes=2, mix_beta=self.args.mix_beta) #.cuda()

        #self.s4_projector = Projector(128, 128).cuda()
        #self.s4_projector = Projector(128, 128).cuda()
        self.s20_projector = Projector(128, 128).cuda()
        #self.s25_projector = Projector(128, 128).cuda()
        ...
        # cl_params = []
        # for name, para in self.s20_projector.named_parameters():
        #     cl_params.append(para)
        ...
        self.exp_lr_scheduler = lr_scheduler.MultiStepLR(self.optimizer, milestones=[150,300,450,500],gamma=0.33,last_epoch= -1)
        # self.cl_lr_scheduler = lr_scheduler.MultiStepLR(self.cl_optimizer,milestones=[50,100,150,200,400,500,550],gamma=0.33,last_epoch= -1 )
    ...
    def train(self):
        ...
        for _, epoch in enumerate(range(self.args.start_epochs, self.args.epochs)):
            ...
            # 通过原始特征获得的损失项目
            # cla_loss +  clu_loss + clu_center_loss + fusion_rep_loss + cl_s20_loss
            ...
            total_d_max, total_d_middle, total_d_min, total_threshold = [], [], [], []
            ...
            for i, (spec, label) in enumerate(tqdm(self.train_data_loader,  desc=' training process')):
                ...
                if args.target_type == 'grad_block':
                    ...
                    #proj1_s4  = self.s4_projector(s20_glo_vec[0])
                    proj1_s20 = self.s20_projector(s20_glo_vec)
                    #proj1_s25 = self.s25_projector(s20_glo_vec[2])


                # mix_features  相比与原始 的features,  需要经过一个 projector() 层；
                # cl_info: list[0]:bt_label, list[1]:bt_index, [2]: lam_ratio;  [3]: mix_glo_vec
                # hyb_clu_loss, hyb_sim_loss, s4_cl_info, s20_cl_info, s25_cl_info =self.net(spec_data, group_mix=True, label=label, )
                ...
                # 3个编码向量，用来投票选出次数最多的； 若是三个都不同，则选择融合网络的输出作为预测结果；
                preds = fus_pred
                preds = preds.cuda()
                ...
                # clu_center_losses.append(clu_center_loss.data.cpu().numpy())
                ...
                if i % 10000 == self.train_data_loader.__len__()-1:
                    ...
                    if epoch > 10 and  best_acc < acc:
                        ...
                        torch.save(self.net.module.state_dict(), args.model_path + '/lab8-6CycGuradin_cls2' +'epoch_'+ str(epoch)+'Acc_'+str(best_acc)  +'.pkl')
            ...
            # self.writter_train.add_scalars("three distance and it's Thread Value",
            #                                { "dmin": np.mean(total_d_min),
            #                                  "dmax": np.mean(total_d_max),
            #                                   "dmiddle": np.mean(total_d_middle),
            #                                   "Thread": np.mean(total_threshold)},
            #                                epoch)

            self.exp_lr_scheduler.step()
            # self.cl_lr_scheduler.step()
            print(f"best_Se{best_Se}\tbest_Sp{best_Sp}\tbest_Sc{best_Sc}\tbest_Acc{best_acc}")
            print(f"ds combine best_Confusion_matrix:\n{best_Confusion_Matrix}")


    # self.evaluate(self.net, epoch, i)
    def evaluate(self, net, epoch, iteration):
        ...
        for i, (spec,label) in enumerate(self.val_data_loader, ):
            ...
            #  聚类损失        #  聚类中心向量的损失；   以及 四个簇表示向量之间的损失；
            clu_loss = self.p_cluster * (ori_s20_clu_loss )  # 1000

            # clu_center_loss = self.p_sim * (center_loss)
            fusion_rep_loss = self.p_sim * (cluster4_fusion_vec_loss )  #

            # 总损失= 原始特征的（聚类损失，相似度约束损失）+ 分类损失  +   混合特征的（聚类损失，相似度约束损失） + 对比学习损失(s20, 10)；
            # 分类损失 + 对比损失 +  原始特征的相似度约束损失 +  混合特征的相似度约束损失；
        ...
        # print("\n Val Accuracy on spec view: {}".format(spec_view_running_corrects.double() / denom))
        # print("Val Accuracy on cof view: {}".format(cof_view_running_corrects.double() / denom))
        # print("Val Accuracy on fusion view: {}".format(fusion_view_running_corrects.double() / denom))

        print("\n -----show the validation info -----------")
        get_score(class_hits, class_counts, True)

        # note, 以下用于在写入tensorboard 可视化中；
